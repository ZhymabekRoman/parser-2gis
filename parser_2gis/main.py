...
def parse_arguments() -> tuple[argparse.Namespace, Configuration]:
    ...
    main_parser.add_argument(
        ...
        help="URL с выдачей или путь к текстовому файлу с URL",
    )
    main_parser.add_argument(
        "-o",
        "--output-path",
    ...
    if args.url and os.path.isfile(args.url[0]):
        with open(args.url[0], "r") as file:
            args.url = [line.strip() for line in file if line.strip()]
